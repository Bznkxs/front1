...
class LabelImage {
	...
	SetImage = (src, memory=false) => {
		console.log("annotate:SetImage (memory=" + memory + ")")
		this.imageMirror = src;
		let _nodes = this.Nodes;
		_nodes.image = new Image();
		_nodes.image.crossOrigin = 'anonymous';
		_nodes.image.src = 'data:image/jpeg;base64,' + src.imageVal;
		//监听图片加载
		_nodes.image.addEventListener('load', () => {
			...
			if (memory) {
				...
				this.Arrays.imageAnnotateMemory = [];
				console.log("src.labelVal", src.labelVal)
				src.labelVal.forEach(label => {
					const midX = label.coords[0], midY = label.coords[1];
					const width = label.coords[2], height = label.coords[3];
					const left = midX - width / 2, top = midY - height / 2;
					const xMin = left * this.iWidth;
					const yMin = top * this.iHeight;
					const xWidth = width * this.iWidth;
					const yHeight = height * this.iHeight;
					this.createNewRectLabel(xMin, yMin, xMin + xWidth, yMin + yHeight, false, label.class, label.prob, label.angle);
				});
			}
	...
	CanvasMouseMove = (e) => {
		...
		if (_arrays.selectIndex !== -1) {
			// console.log("CanvasMouseMove", _arrays.selectIndex)
			let imageIndexShow = _arrays.imageAnnotateShower[_arrays.selectIndex].content;
	...
	CanvasMouseDown = (e) => {
		...
		if (e.button === 0) {
			this.isDragCircle = false;
			if (_arrays.selectIndex !== -1 && !readOnly) {
				let imageIndex = _arrays.imageAnnotateShower[_arrays.selectIndex].content;
				if (imageIndex.length > 0) {
					for (let i=0; i<imageIndex.length; i++) {
						...
						if (distanceFromCenter <= this.radius){
							...
							if (_arrays.imageAnnotateShower[_arrays.selectIndex].contentType === "rect") {
							...
							// else if (_arrays.imageAnnotateShower[_arrays.selectIndex].contentType === "polygon") {
			...
			if (!this.isDragCircle){
				...
				else if (this.Features.rectOn && !readOnly) {
					// 是否开启绘制矩形功能
					console.log("rectOn")
					this.Arrays.selectIndex = -1;
					_nodes.ctx.lineWidth = 1;
				...
				// 			_arrays.selectIndex = resultList[i].id - 1;
				// 			isActive = true;
				// 		}
				// 	}
				// 	if (!isActive) {
				// 		_arrays.selectIndex = -1;
				// 	}
				// 	if (_arrays.selectIndex === -1) {
				// 		// 未选定标签结果，创建新标签
	...
	CalcRectMask = (arrays) => {
		if (arrays.length >= 2) {
			...
			this.Arrays.imageAnnotateShower[this.Arrays.selectIndex].rectMask = {
				"xMin": xMin,
				"yMin": yMin,
				"width": xMax - xMin,
				"height": yMax - yMin
			...
			this.Arrays.imageAnnotateShower[this.Arrays.selectIndex].labelLocation.x = labelX;
			this.Arrays.imageAnnotateShower[this.Arrays.selectIndex].labelLocation.y = labelY;
		}
	};
	...
	//----绘制矩形的方法
	DrawRect = (ctx, x, y, width, height, angle, color, rgb, lineWidth, opacity) => {
		if (lineWidth != null) {
		...
		if (angle === 0) {
			...
			let px = (width * t * t - height * t) / (t * t - 1);
			let py = px / t;
			// console.log(angle * 180 / Math.PI, t, px, py)
			ctx.moveTo(x+px, y);
			ctx.lineTo(x+width, y+height-py);
			ctx.lineTo(x+width-px, y+height);
			ctx.lineTo(x, y+py);
			ctx.lineTo(x+px, y);
			ctx.fill();
			ctx.stroke();
	...
	DrawSavedAnnotateInfoToShow = (enterIndex, mouseMoveOnly=false, forceRepaint=false) => {
		...
		_arrays.imageAnnotateShower.forEach((item, index) => {
			if (item.contentType === "polygon") {
				...
				if (item.labels.prob >= probThreshold) {
					this.DrawRect(_nodes.ctx, item.rectMask.xMin, item.rectMask.yMin,
						item.rectMask.width, item.rectMask.height,
						item.labels.angle,
						item.labels.labelColor, item.labels.labelColorRGB,
						lineWidth,
						opacity);
				} else {
					// console.log(item.labels.prob, probThreshold)
				}

			}
			if (_arrays.selectIndex !== -1 && _arrays.selectIndex === index && !readOnly) {
			...
			// 绘制矩形蒙层
			if (enterIndex && enterIndex-1 === index) {
				_nodes.ctx.beginPath();
	...
	CircleDrag = (e) => {
		this.GetMouseInCanvasLocation(e);
		let imageIndex = this.Arrays.imageAnnotateShower[this.Arrays.selectIndex].content;
	...
	ImageDrag = (e) => {
		...
		if (this.dragRectMask != null && !readOnly) {
			// console.log("Drag rect mask", this.dragRectMask);
			let imageIndex = this.Arrays.imageAnnotateShower[this.dragRectMask].content;
			// console.log(imageIndex, offsetX, offsetY);
			for (let i in imageIndex) {
				imageIndex[i].x += offsetX;
				imageIndex[i].y += offsetY;
			}
			// this.setPolygonXY(imageIndex, offsetX, offsetY);
	...
	DragRectCircleRepaintRect = (e) => {
		...
		this.GetMouseInCanvasLocation(e);
		let imageIndex = this.Arrays.imageAnnotateShower[this.Arrays.selectIndex].content;
		this.Nodes.ctx.fillStyle = "rgba("+ this.Arrays.imageAnnotateShower[this.Arrays.selectIndex].labels.labelColorRGB +","+ this.opacity +")";
		imageIndex[this.snapCircleIndex].x = this.mouseX;
		imageIndex[this.snapCircleIndex].y = this.mouseY;
		// console.log(this.snapCircleIndex, imageIndex)
	...
	createNewRectLabel = (xMin, yMin, xMax, yMax, shower=true, labelName='0', prob=1, angle=0) => {
		// console.log("createNewRect")
		if (xMin > xMax) {
			let t = xMin;
			xMin = xMax;
			xMax = t;
		...
		shower?	this.ReplaceAnnotateMemory(): this.ReplaceAnnotateShow();
	}
	//----创建新的标定结果标签
	...
	findNearest = (os, preferredClass) => {
		...
		for (let i in this.Arrays.imageAnnotateShower) {
			...
			for (let oN in os) {
				...
				let cls = this.Arrays.imageAnnotateShower[i].labels.labelName;
				let dist = distSquared(coords, o);
				if ((preferredClass != null && retClass !== preferredClass && cls === preferredClass) || (ret == null || dist < minDist)) {
					ret = +i;
					minDist = dist;
					retClass = cls;
				}
	...
	deleteMultipleLabels = (index) => {
		...
		let coords = [];
		let rects = [];
		let classname = this.Arrays.imageAnnotateShower[index].labels.labelName;
		for (let i in this.Arrays.imageAnnotateShower) {
		...
		while (qP < queue.length) {
			...
			++qP;
			for (let i in this.Arrays.imageAnnotateShower) {
				if (!remove_map[i] && this.Arrays.imageAnnotateShower[i].labels.labelName === classname && this.checkMinDist(rectDistSquared(o, rects[i]), false)) {
					queue.push(i);
					remove_map[i] = 1;
	...
	DeleteSomeResultLabel = (index, isRecursive=false) => {
		...
		let removedLabelClass = this.Arrays.imageAnnotateShower[index].labels.labelName;
		...
		let nearest = this.findNearest(this._deletedPositions, removedLabelClass);
	...
	//----已标定结果列表交互操作
	ResultListOperation = () => {
		console.log("result list operation")
		this.Nodes.resultGroup.onclick = (event)=>{
			console.log("result list click")
		...
		for (let i=0; i<resultList.length; i++) {
			...
			resultList[i].getElementsByClassName('result_Name').onclick = function() {
				console.log("result name click")
			}

			resultList[i].onmouseenter = function() {
				console.log("resultList[" + i + '].mouseOver')
			...
			resultList[i].onclick = function(event) {
				...
				console.log("resultList[" + i + '].click')
				...
				switch (target.classList[0]) {
					...
					case "result_Name":
						...
						_self.Arrays.selectIndex = this.index;
						_self.DrawSavedAnnotateInfoToShow();
						break;
					case "isShowLabel":
	...
	recover = () => {
		setStorage(imgArray[imgIndex].name, null)
		this.imageMirror.labelVal = this.imageMirror.labelOriginal;
		this.SetImage(this.imageMirror);
	};
	//----全屏显示事件
	IsScreenFull = () => {
	...
	//----calculate position (limited by canvas)
	calcXY = (vx, vy) => {
		let o = {x: vx, y: vy};
		if (vx < this.appearSize - this.iWidth * this.scale) {
			o.x = this.appearSize - this.iWidth * this.scale;
		}
		else if (vx > this.cWidth - this.appearSize) {
			o.x = this.cWidth - this.appearSize;
		}

		if (vy < this.appearSize - this.iHeight * this.scale) {
			o.y = this.appearSize - this.iHeight * this.scale;
		}
		else if (vy > this.cHeight - this.appearSize) {
			o.y = this.cHeight - this.appearSize;
		}
		return o;
	}
	setPolygonXY = (polygon, offsetX, offsetY) => {
		let repX, repY;
		if (offsetX > 0) repX = -1e100; else repX = 1e100;
		if (offsetY > 0) repY = -1e100; else repY = 1e100;
		console.log("in setPolygonXY")
		console.log(polygon);
		for (let i in polygon) {
			console.log(i, polygon[i].x, polygon[i].y)
			if (offsetX > 0) {
				if (repX < polygon[i].x) repX = polygon[i].x;
			} else {
				if (repX > polygon[i].x) repX = polygon[i].x;
			}
			if (offsetY > 0) {
				if (repY < polygon[i].y) repY = polygon[i].y;
			} else {
				if (repY > polygon[i].y) repY = polygon[i].y;
			}
		}
		let o = this.calcXY(repX + offsetX, repY + offsetY);
		let finalOffsetX = o.x - repX;
		let finalOffsetY = o.y - repY;
		console.log("rep", repX, repY, finalOffsetX, finalOffsetY);
		console.log(polygon);
	...
	ReplaceAnnotateMemory = () => {
		this.Arrays.imageAnnotateMemory.splice(0, this.Arrays.imageAnnotateMemory.length);  // remove all
...
1486: ReplaceAnnotateMemory()
1512: ReplaceAnnotateShow()


Arrays.imageAnnotateMemory = [  // original data annotation
	{
		content: [
			{x, y},
			...
		...
		labels,
		labelLocation: ComputerLabelLocation(rectMask),  // midX, midY
		contentType
	}
]

Arrays.imageAnnotateShower = [  // shown data annotation
	{
		content: [
			{x, y},
			...
